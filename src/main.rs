use mithril_client::{AggregatorDiscoveryType, ClientBuilder, GenesisVerificationKey, MessageBuilder, MithrilResult};
use serde_json::json;
...
async fn main() -> MithrilResult<()> {
    ...
    let client = ClientBuilder::new(AggregatorDiscoveryType::Url(AGGREGATOR_ENDPOINT.to_string())).set_genesis_verification_key(
    ...
    // input
    let transactions_hash = "22d4a83d2e823c28c9d531d135cd12fe8d0d2a33140cac7b446a11888d37ff47";

    // output
    let cardano_transaction_proof = client
        .cardano_transaction()
        .get_proofs(&[transactions_hash])
        .await?;
    ...
    println!("{}", json!({"proof": cardano_transaction_proof, "certificate": certificate}));
